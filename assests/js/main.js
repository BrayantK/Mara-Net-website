...
var homeSwiper = new Swiper(".home-swiper", {
    spaceBetween: 30,
    loop: 'true', // Add missing semicolon here
...
const shopItems = document.querySelectorAll(".shop_content");

// Function to filter shop items based on selected filters
function filterShopItems() {
  const selectedFilters = [];

  // Loop through all checkboxes and collect selected filters
  filterCheckboxes.forEach(checkbox => {
    if (checkbox.checked) {
      selectedFilters.push(checkbox.nextElementSibling.textContent.trim()); // Get filter text
    }
  });

  // Loop through all shop items
  shopItems.forEach(shopItem => {
    const shopSubtitles = shopItem.querySelectorAll(".shop_subtitle");
    let showItem = false; // Flag to determine if item should be shown

    // Check if any subtitle of the shop item matches a selected filter
    shopSubtitles.forEach(subtitle => {
      if (selectedFilters.includes(subtitle.textContent.trim())) {
        showItem = true;
        return; // Exit loop if a match is found
      }
    });

    // Show/hide shop item based on the flag
    shopItem.style.display = showItem ? "flex" : "none";
  });
}

// Add event listener to all filter checkboxes for dynamic filtering
filterCheckboxes.forEach(checkbox => {
  checkbox.addEventListener("change", filterShopItems);
});

// Call filter function on page load (optional)
filterShopItems();
